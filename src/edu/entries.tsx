...
export const ENTRIES: Entry[] = [
  ...
  // ------------------------------------------------------------------ fiscal
  {
    ...
    model: 'pipeline += 1.0·Δ(spending % GDP) − 0.6·Δ(tax % GDP);  impulse = 40% of pipeline per quarter, capped ±1.5\ncyclical revenue = −0.35·gap;  cyclical benefits = 0.5·max(0, unemployment − NAIRU)',
  ...
  {
    id: 'fiscal-rules',
    title: 'Fiscal rules and the OBR',
    category: 'Fiscal',
    summary: 'A rule the Chancellor sets for themselves. Meeting it buys cheaper borrowing; breaking it costs more than never having one.',
    body: [
      'Since 1997 every Chancellor has bound themselves with a rule: a deficit ceiling, a current-budget balance, or debt falling. The Office for Budget Responsibility judges whether the forecast meets it. The point is not the arithmetic but the signal: a government that keeps its own rule is a government the gilt market lends to cheaply.',
      'In the game you choose a rule in the budget panel. It is judged from 2029, the end of the parliament, and every quarter after that in breach adds to the risk premium and dents business confidence. Meeting it takes a little off the premium. Having no rule at all costs a permanent quarter-point. Changing the rule mid-parliament costs credibility and resets nothing.',
      'The three rules bite differently. The 3% deficit rule is simple and bites in recessions, exactly when you want to borrow. The current-budget rule lets you borrow to invest, so infrastructure and green spending are exempt. The debt-falling rule is judged year on year and rewards growth as much as restraint.',
    ],
    model: 'headroom: stability = 3 − deficit;  investment = infrastructure + green − deficit;  debt = debtRatio(last year) − debtRatio\npremium += breached ? 0.05 + 0.02·breachQuarters : met ? −0.15 : 0;  none: +0.2',
    toWin: ['Pick the rule that matches your plan: investors, take the current-budget rule; consolidators, the deficit rule; growth strategies, debt falling.', 'A breach in a recession is forgivable if you fix it by the next Budget. A breach you rewrite is remembered.'],
    related: ['debt-dynamics', 'risk-premium', 'parliament'],
    metrics: ['ruleHeadroom', 'giltYield'],
    figure: (h) => <Lines history={h} yLabel="% GDP / pp" series={[{ name: 'Rule headroom', color: C.green, get: (s) => s.ruleHeadroom }, { name: 'Gilt premium', color: C.red, get: (s) => s.riskPremium }, { name: 'Deficit', color: C.amber, get: (s) => s.deficit }]} />,
  },
  {
    id: 'parliament',
    title: 'Parliament and the majority',
    category: 'Politics',
    summary: 'A Prime Minister governs only with the consent of their own MPs. Big programmes need a vote, and rebels come from low unity.',
    body: [
      'Your Commons majority is set at each election from the gap between your share and the opposition\'s. It shrinks with by-election losses and defections. A big budget (policy friction above 1.5 in a quarter) or any assault on the courts, the press or the Bank triggers a vote.',
      'Rebels come from three places: a demoralised party (unity below 55), the size of the swing you are asking them to vote for, and institutional damage, which the liberal wing will not wear. Every rebel is a vote lost and a vote gained by the other side, so you lose when rebels exceed half your majority. A defeat waters the programme down by half and costs unity and trust.',
      'When the majority falls into single figures the confidence-motion card appears. You can buy a smaller party with policy or face the vote and hope your party is united. This is the legitimacy loop from the systems map, made concrete: approval feeds unity, unity feeds votes, votes feed policy, policy feeds approval.',
    ],
    model: 'rebels = 0.5·max(0, 55 − unity) + 6·friction + 2.5·institutionalDamage + noise\nwin if 2·rebels < majority;  defeat: levers ← prev + ½·(levers − prev), unity −4, trust −1',
    toWin: ['Spread a consolidation over several budgets: three small votes you win beat one big vote you lose.', 'Keep unity above 60 before any institutional card. Below that, the courts option loses you the vote and the moral high ground.'],
    related: ['unrest', 'elections', 'institutions', 'voter-blocs'],
    metrics: ['majority', 'partyUnity'],
    figure: (h) => <Lines history={h} yLabel="seats / index" series={[{ name: 'Majority', color: C.blue, get: (s) => s.majority }, { name: 'Party unity', color: C.green, get: (s) => s.partyUnity }, { name: 'Rebels (last vote)', color: C.red, get: (s) => s.lastVote?.rebels ?? 0 }]} />,
  },
];
