...
export const METRIC_GROUPS: { title: string; metrics: MetricDef[] }[] = [
  {
    title: 'Economy',
    metrics: [
      { key: 'growth', learn: 'output-gap', label: 'GDP growth (ann.)', get: (s) => s.growth, fmt: pct(1), dir: 1, help: 'Annualised real growth this quarter. Potential growth ≈ productivity + labour force.' },
      { key: 'inflation', learn: 'phillips-curve', label: 'Inflation', get: (s) => s.inflation, fmt: pct(1), dir: -1, help: 'CPI. Driven by expectations, the output gap, energy and sterling. Target 2%.' },
      { key: 'unemployment', learn: 'okun', label: 'Unemployment', get: (s) => s.unemployment, fmt: pct(1), dir: -1, help: "Okun's law around a structural rate (NAIRU) that welfare and skills move." },
      ...
      { key: 'deficit', learn: 'fiscal-multiplier', label: 'Deficit', get: (s) => s.deficit, fmt: pct(1), dir: -1, help: 'Spending minus revenue, % GDP. Includes debt interest and cyclical benefits.' },
      { key: 'debtRatio', learn: 'debt-dynamics', label: 'Debt', get: (s) => s.debtRatio, fmt: pct(0), dir: -1, help: 'National debt, % GDP. Above ~90% the gilt risk premium climbs.' },
  ...
  },
  {
    title: 'Society',
    metrics: [
      { key: 'happiness', learn: 'happiness', label: 'Gross national happiness', get: (s) => s.happiness, fmt: num(0), dir: 1, help: 'Composite of income, jobs, prices, NHS, crime, housing, inequality, cohesion, trust.', bar: true },
      { key: 'nhsQuality', learn: 'nhs-need', label: 'NHS quality', get: (s) => s.nhsQuality, fmt: num(0), dir: 1, help: 'Funding relative to need. Need grows ~1.5%/yr with ageing.', bar: true },
      ...
      { key: 'housePriceToIncome', learn: 'housing', label: 'House price / income', get: (s) => s.housePriceToIncome, fmt: num(1), dir: -1, help: 'Affordability. Population growth vs construction, plus credit.' },
      { key: 'netMigration', learn: 'migration', label: 'Net migration', get: (s) => s.netMigration, fmt: (v) => `${v.toFixed(0)}k`, dir: 0, help: 'Per year. Openness × how attractive the UK economy is right now.' },
      ...
      { key: 'cohesion', learn: 'integration', label: 'Social cohesion', get: (s) => s.cohesion, fmt: num(0), dir: 1, help: 'Trust between groups. Integration, inequality, unemployment, crime.', bar: true },
  ...
  },
  {
    title: 'Institutions',
    metrics: [
      { key: 'trust', learn: 'corruption', label: 'Trust in government', get: (s) => s.trust, fmt: num(0), dir: 1, help: 'Slow to build, quick to lose. Scandals, inflation, policy churn.', bar: true },
  ...
  },
  {
    title: 'Politics',
    metrics: [
      { key: 'nationalApproval', learn: 'voter-blocs', label: 'Approval', get: (s) => s.nationalApproval, fmt: pct(0), dir: 1, help: 'Size-weighted bloc approval. You need roughly 43%+ at the poll to win.', bar: true },
      { key: 'partyUnity', learn: 'unrest', label: 'Party unity', get: (s) => s.partyUnity, fmt: num(0), dir: 1, help: 'Below 30 and the letters go in.', bar: true },
      { key: 'unrest', learn: 'unrest', label: 'Unrest', get: (s) => s.unrest, fmt: num(0), dir: -1, help: 'Above 80 for three quarters and you resign.', bar: true },
